...
function verify(vcode) {
    if(($("input:checkbox:checked").val() != '01')){return;}
    function successFn(res) {
        ...
        if (res.code == 1) {
            ...
            if (code == 1) {
                ...
                if (acID == 'JS0001DS001') {
                    ...
                    $pop.find('.box').append('<p class="left-txt">当前拥有' + leftTimes + '次拆红包机会</p>');
                ...
                if (acID == 'JS0001DS001') {
                    ...
                    info = info && info.replace(/\n|，/g, '<br>');
                    $pop.find('.box').append('<p class="right-txt-l">' + info + '</p>');
                    $pop.find('.box').append('<p class="left-txt">请核对是否输入正确。</p>');
...
$('.explain span').on('click', function() {
    $('.shenmin').show();
})
